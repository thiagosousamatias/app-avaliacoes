-- Troca o filtro do dashboard de igualdade exata para faixa (min/max), pra suportar filtros
-- dicotomizados como "saude_fisica Ruim" (valores 1-3) sem quebrar filtros de valor unico
-- existentes (min=max=valor continua funcionando identico a antes).
        ...
        or not exists (
          ...
          where not exists (
            ...
            where ri.sessao_id = s.id
              ...
              and ri.valor_numerico between (f->>'min')::numeric and (f->>'max')::numeric
          )
        )
      )
  ),
